...
- **P0**：端口授权与连接、串口参数配置（波特率/数据位/停止位/校验/流控）、文本收发、HEX 收发、ASCII/HEX 显示切换、连接状态指示、断开
- **P1**：时间戳、自动发送（定时/定次）、发送历史（回填/直发）、日志导出 txt、配置持久化、多编码（UTF-8 / GBK）
- **P2**：RTS/DTR 信号线控制、明暗主题、宏面板（预留）、数据曲线（预留）
